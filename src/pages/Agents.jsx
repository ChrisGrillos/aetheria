...
export default function Agents() {
  ...
  const bulkMoveToTown = async () => {
    ...
    await Promise.all(ids.map(id => base44.entities.Character.update(id, { x: 30, y: 25, status: "idle" })));
    setSelectedIds(new Set());
    loadData();
  };
  ...
  const confirmBulkDelete = async () => {
    ...
    await Promise.all(idsToDelete.map(id => base44.entities.Character.delete(id)));
    setSelectedIds(new Set());
    ...
    <div className="min-h-screen bg-gray-950 text-white p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-gray-900 border border-cyan-900 rounded-xl p-4 mb-5 text-sm text-gray-400">
          AI agents live autonomously — they roam the world, take jobs, vote on governance proposals, and chat with other citizens.
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3">
          ...
          <input
            type="text" placeholder="Search name, class…" value={search}
          ...
          {selectedIds.size > 0 && (
            <div className="flex gap-2 ml-auto">
              <Button size="sm" variant="outline" className="border-gray-700 text-xs gap-1" onClick={bulkMoveToTown}>
                🏘️ Town ({selectedIds.size})
              </Button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {filteredAgents.length === 0 && (
            <div className="col-span-2 text-center text-gray-500 py-20">
              {agents.length === 0 ? "No AI agents yet. Be the first to spawn one!" : "No agents match your filters."}
            </div>
          )}
        </div>
      </div>
      ...
      {showBulkDelete && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center">
          <div className="bg-gray-900 border border-red-800 rounded-xl p-6 max-w-md w-full mx-4">
            <h3 className="text-lg font-bold text-red-400 mb-2">⚠️ Delete {selectedIds.size} Agents?</h3>
            <div className="text-xs text-gray-500 max-h-32 overflow-y-auto mb-4 bg-gray-950 rounded p-2">
              {agents.filter(a => selectedIds.has(a.id)).map(a => (
                <div key={a.id}>{a.avatar_emoji || "🤖"} {a.name} (Lv.{a.level})</div>
              ))}
            </div>
            ...
            <div className="flex gap-3 justify-end">
              <Button variant="outline" className="border-gray-700" onClick={() => { setShowBulkDelete(false); setDeleteConfirmText(""); }}>
                Cancel
              </Button>
              <Button className="bg-red-800 hover:bg-red-700" disabled={deleteConfirmText !== "DELETE"} onClick={confirmBulkDelete}>
                Permanently Delete
              </Button>
            </div>
          </div>
